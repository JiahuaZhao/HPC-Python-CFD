...
# Import the external jacobi function from "jacobi_numpy_ne.py"
from jacobi_numpy_ne import jacobistepvort, deltasq, jacobistep
...
def main(argv):
    ...
    # better than double for-loop:
        bnorm = 0
    bnorm += np.sum(psi * psi)  # this is not working, just keep for the moment the iterative version
    ...
    if not irrotational:
        ...
        zet = _boundaryzet(zet, psi, m, n)
        # update normalisation
        # for i in range(1, m+2):
        #    for j in range(1, n+2):
        #        bnorm += zet[i][j]*zet[i][j]
        # better than double for-loop:
        bnorm += np.sum(zet * zet)
        # iterative:
        #for i in range(m + 2):
        #    for j in range(n + 2):
        #        bnorm += zet[i][j] * zet[i][j]
